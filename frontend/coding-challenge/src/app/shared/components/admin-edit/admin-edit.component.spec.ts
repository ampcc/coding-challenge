...
describe('AdminEditComponent', () => {
  ...
  it('error message for name hidden on default', () => {
    const error = !fixture.debugElement.query(By.css('.name_error')).nativeElement.classList.contains('not_showing');
  ...
  it('error message for description hidden on default', () => {
    const error = !fixture.debugElement.query(By.css('.description_error')).nativeElement.classList.contains('not_showing');
  ...
  it('edit challenge call gets triggered on button click', fakeAsync(() => {
    const editChallenge = spyOn(component, 'editChallenge');
